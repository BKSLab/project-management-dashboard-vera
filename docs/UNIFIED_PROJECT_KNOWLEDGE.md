...
Постоянные диалоги, память обсуждения и выполнение инструментов находятся в [контуре проектного агента](PROJECT_AGENT.md). Таблицы `agent_conversations` и `agent_messages` исключены из общего каталога: личная переписка не индексируется и не подтверждает факты проекта.

Чат и MCP используют общий каталог и объединение FTS/семантических результатов через RRF. Каждый кандидат проверяется по текущему PostgreSQL: удалённые и чужие источники исключаются, актуальные поля читаются заново, устаревший текст не передаётся модели. В стартовом ответе поиска остаются до пяти компактных источников и указатель подходящего фрагмента для дочитывания.

Агент получает краткий паспорт, точные количества источников и релевантные описания. Документы и извлечённые файлы имеют кэш саммари по версии содержимого; полный текст остаётся доступен поиску и инструментам. Команда и связи читаются по необходимости. SQL-инструменты по-прежнему отвечают за расчёт статистики и календарных последствий. Объём каждого раунда ограничен [общим бюджетом контекста](AGENT_CONTEXT_BUDGET.md).

Для продолжения доступны `list_sources`, `read_source`, `related_sources`, `search_sources`. В MCP они объединены в `read_project_knowledge(project_key, request)`. У страниц есть `next_offset`; полное чтение длинного источника продолжается с указанного смещения. По умолчанию чат допускает восемь раундов инструментов, до восьми запросов за раунд, и затем формирует ответ с обозначением оставшихся пробелов. Ссылки принимаются только по выданным сервером хэндлам текущего запроса.

В проектном чате эти возможности зарегистрированы в `AgentToolExecutor`. Модель получает схемы `available_tools` и вызывает инструменты через `tool_calls` с `name` и `arguments`; проект и инициатор передаются обработчику только сервером. Каждая новая реплика заново получает актуальное состояние проекта, отдельно от сохранённой истории разговора.
