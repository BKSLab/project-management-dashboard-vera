...
Обновление 2026-09-21: [проектный чат](PROJECT_AGENT.md) переведён на постоянную очередь. `POST /projects/{id}/agent/conversations/{conversation_id}/messages` сохраняет вопрос и возвращает `202`; результат читается через polling сообщений. Общий срок фоновой попытки `AGENT_TURN_TIMEOUT_SECONDS=600` включает память, планирование, retrieval, все вызовы модели и их повторы. Срок может прервать клиентские повторы; HTTP-запрос не ожидает генерацию и не требует увеличения timeout nginx. Прежний `/knowledge/ask` и остальные синхронные AI-операции сохраняют описанное ниже расхождение. Ограничение изменения API в пункте 2 относилось к прежнему DI-рефакторингу; новые маршруты добавлены в рамках согласованного развития проектного агента.
