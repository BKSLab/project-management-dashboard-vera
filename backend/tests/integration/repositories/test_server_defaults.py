...
@pytest.mark.asyncio
async def test_every_repository_returns_server_defaults_from_the_write_itself(db_session: AsyncSession, user: User, project: Project, stage: ProjectStage) -> None:
    """Все репозитории отдают серверные значения прямо из запроса записи: отдельного дочитывания нет ни у одной сущности."""
...
@pytest.mark.asyncio
async def test_returned_values_match_the_stored_row(db_session: AsyncSession, project: Project, stage: ProjectStage) -> None:
    """Возвращённые значения совпадают с прочитанными заново, а updated_at растёт при обновлении."""
    # Возвращённые значения совпадают с тем, что реально лежит в базе. Именно это гарантирует, что удаление дополнительного SELECT не подменило server-defaults значениями, придуманными в Python.
    repository = TasksRepository(db_session)
    task = await repository.save(
        data={
            "project_id": project.id,
    ...
    # Обновление действительно двигает серверный `updated_at`. Без этого проверка «поле заполнено» прошла бы и на устаревшем значении, оставшемся от вставки.
    repository = DocumentsRepository(db_session)
