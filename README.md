...
- интерактивная карта и редактируемое дерево ИСР с rollup-прогрессом и синхронизацией листовых работ с канбаном;
...
Миниатюра изображения открывает отдельный preview поверх окна задачи; из него доступны скачивание и открытие оригинала. Для маршрутов загрузки и выдачи вложений nginx отключает временное request/response buffering и передаёт данные потоком.

## ИСР и начальные данные

Страница `/wbs` по умолчанию открывает интерактивную SVG-карту. Карта строится из живых данных `/wbs/tree` и `/kanban/stages`, поэтому не зависит от статического draw.io-экспорта. Цвет листовой задачи соответствует стадии канбана; родительские узлы показывают суммарный прогресс и многосегментное распределение стадий. Доступны pan/zoom, обзорная и полная раскладка, фильтры по фазе и стадии, поиск с автоматическим раскрытием пути и открытие модального окна задачи. Переключатель `Карта | Дерево` сохраняет прежний режим редактирования структуры.

Начальный обзор показывает восемь фаз с агрегированным прогрессом. Ветви раскрываются по клику, выбор отдельной фазы сразу показывает её разделы, а команда «Все задачи» отображает полную структуру. Это не перегружает экран при первом открытии и сохраняет доступ ко всем 195 листовым задачам.
