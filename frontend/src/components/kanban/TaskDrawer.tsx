...
export function TaskDrawer({ task, onClose }: TaskDrawerProps) {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Pick<KanbanTask, "description_md" | "due_date">>) =>
            api.patch<KanbanTask>(`/api/kanban/tasks/${task.id}`, data),
        onSuccess: () => {
            invalidateTask();
    ...
    const commentsQuery = useQuery({
        ...
        queryFn: () => api.get<TaskComment[]>(`/api/kanban/tasks/${task.id}/comments`),
    });

    const activityQuery = useQuery({
        queryKey: ["kanban", "tasks", task.id, "activity"],
        queryFn: () => api.get<TaskActivity[]>(`/api/kanban/tasks/${task.id}/activity`),
    });

    const linksQuery = useQuery({
        queryKey: ["kanban", "tasks", task.id, "links"],
        queryFn: () => api.get<LinkedDocument[]>(`/api/kanban/tasks/${task.id}/links`),
    });
    ...
    const linkMutation = useMutation({
        mutationFn: (documentId: number) =>
            api.post("/api/document-links", { document_id: documentId, kanban_task_id: task.id }),
        onSuccess: () => {
            setSelectedDocId("");
            invalidateLinks();
        },
    ...
    const unlinkMutation = useMutation({
        mutationFn: (linkId: number) => api.delete(`/api/document-links/${linkId}`),
        onSuccess: invalidateLinks,
    });

    const addCommentMutation = useMutation({
        mutationFn: () =>
            api.post<TaskComment>(`/api/kanban/tasks/${task.id}/comments`, {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onClick={onClose}>
            <div
                className="h-full w-full max-w-lg overflow-y-auto border-l border-white/[0.06] p-6 shadow-[var(--shadow-panel)] backdrop-blur-xl"
                style={{ backgroundColor: "rgba(26,31,46,0.85)" }}
            ...
            >
                <div className="mb-4 flex items-start justify-between gap-4">
                    ...
                    <button
                        onClick={onClose}
                        aria-label="Закрыть"
                        className="shrink-0 rounded-lg p-1.5 text-muted hover:bg-white/10 hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
                    >
                        ✕
                    </button>
                </div>
                ...
                <section className="mb-4 rounded-xl border border-white/[0.05] bg-surface-elevated p-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <select
                            ...
                            className="min-w-48 flex-1 rounded border border-border bg-surface px-2 py-1.5 text-sm text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
                        >
