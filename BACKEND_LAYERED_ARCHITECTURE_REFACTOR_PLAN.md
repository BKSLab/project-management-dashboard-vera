...
worker-а и за 11,5 часов раздул временный файл захвата вывода pytest в
`%TEMP%` до 349 ГБ, заполнив диск C: до нуля. Следствия: Docker Desktop
перестал отвечать, PowerShell падал на записи вывода, Chrome потерял
раскладку закреплённых вкладок. Процесс снят, место возвращено
(файл был delete-on-close). Вывод для будущих прогонов: не оставлять
`pytest` фоновой задачей без проверки кода возврата, а тест, вызывающий
`KnowledgeWorker.run`, обязан гарантированно взводить `stop_event`.
Размеры каталогов на этой машине мерить `robocopy /L /S /XJ /BYTES`:
`Get-ChildItem -Recurse` недосчитал 350 ГБ и увёл диагностику в ложную
сторону.
...
### Этап 5. Сделать task document import одной транзакцией — ✅ выполнен

Импорт получил `UnitOfWork` и хранилище файлов обязательными зависимостями.
Вложенные сервисы вызываются с явным `commit=False`, финальный commit —
один. Компенсирующее удаление уже закоммиченных строк удалено: их снимает
откат. Компенсируется только физический файл, который транзакция откатить
не может.

Ключ хранения понадобился для этой компенсации, но в публичной схеме его
нет. Поэтому у сервиса файлов появился `save_in_transaction`, возвращающий
`StoredAttachment` с ключом; самостоятельный `upload_attachment` остался с
прежней сигнатурой и владеет своим commit.

`KnowledgeEvents` стал обязателен в восьми мутирующих сервисах, а
`TaskAttachmentStorage` — в `ProjectsService` и `TasksService`. Снято 22
проверки `if ... is not None`, молча отключавшие transactional outbox.
Один unit-тест после этого упал: он проходил только потому, что outbox был
выключен, и дублёр задачи не знал своего проекта.

Атомарность подтверждена на PostgreSQL: при сбое последнего шага в базе не
остаётся ни одной из трёх строк и ни одного файла-сироты.

Итог: `1037 passed`, `ruff All checks passed`.
...
### Этап 6. Устранить удержание DB ресурсов в streaming и AI use cases — ✅ выполнен

**Выдача файла.** Аутентификация, проверка доступа и чтение метаданных
выполняются в короткой области внутри `AttachmentDownloadService`. В графе
маршрута не осталось ни `get_db_session`, ни построенных на нём сервисов —
в фазу передачи уходит только неизменяемый `AttachmentDownload`.

**AI-сценарии.** Analytics, Project Agent, task rephrase, WBS suggestion,
импорт документа и MCP semantic search разделены на фазы: короткое чтение →
закрытие области → внешний вызов → при необходимости короткая запись.
Сервисы получают фабрику области вместо репозиториев и о SQLAlchemy не
знают; SQLAlchemy-реализация живёт в `dependencies/scopes.py`.

Область описана конкретным набором именованных репозиториев
(`services/db_scope.py`), а не контейнером с поиском по имени.

**Найдена и исправлена ошибка:** после этапа 2 эндпоинт `/knowledge/ask`
передавал `project_id`, а `ProjectAgentService.ask` ожидал ORM-модель
проекта. Этот путь падал бы в рантайме; в тестах он был закрыт заглушкой
сервиса. Теперь сервис принимает идентификатор и читает проект сам.

Инвариант проверяется двумя способами: unit scope-tracker убеждается, что
в момент вызова модели область уже закрыта, а integration-тест с пулом из
двух соединений и заблокированным внешним клиентом показывает, что обычный
запрос к базе продолжает отвечать.

Синхронные API-контракты сохранены согласно решению этапа 0.

Итог: `1071 passed`, `ruff All checks passed`.
...
### Этап 10. Закрепить архитектуру тестами — 🟨 выполнен частично
